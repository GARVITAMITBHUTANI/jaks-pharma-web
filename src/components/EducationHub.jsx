...
const EducationHub = () => {
  ...
  return (
    <section id="education" className="py-24 bg-[#F9FAFB]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row justify-between items-end mb-12 reveal">
          <div className="max-w-2xl">
            ...
            <h2 className="text-3xl sm:text-4xl font-extrabold text-[#111827]">Insights & Knowledge</h2>
          </div>
          <Link to="/articles" className="mt-6 md:mt-0 inline-flex items-center gap-2 text-[#00529B] font-semibold hover:text-[#F37021] transition-colors group cursor-pointer">
            View All Articles
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 reveal">
          {filteredArticles.slice(0, 3).map((article, index) => (
            <div key={article.id} className={`group bg-white rounded-2xl border border-gray-100 overflow-hidden hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-all duration-300 hover:-translate-y-1 delay-${index * 100} flex flex-col`}>
              ...
              <div className="p-6 flex flex-col flex-grow">
                ...
                <div className="mt-auto">
                  <Link to={`/article/${article.id}`} className="group/btn inline-flex items-center text-sm font-semibold text-[#F37021] hover:text-[#D9611B] transition-colors">
                    {article.category === 'Videos' ? 'Watch Video' : 'Read Article'}
